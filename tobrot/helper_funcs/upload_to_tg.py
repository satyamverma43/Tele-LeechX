    ...
    if os.path.isdir(local_file_name):
        ...
        if not message.photo:
            new_m_esg = await message.reply_text(
                f"<b>🛠 Extracting : </b> <code>{len(directory_contents)}</code> <b>File(s) <a href='tg://user?id={from_user}'></a></b>",
                quote=True
