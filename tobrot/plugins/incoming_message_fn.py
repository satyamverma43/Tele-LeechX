    ...
    if rep_mess:
        ...
        if not rep_mess.media or str(file_name).lower().endswith(".torrent"):
            ...
            if user_command == LEECH_COMMAND.lower():
                u_men = message.from_user.mention
                await i_m_sefg.edit(f"<b> Hey {u_men}, \n\n ⚠️ Check and Send a Valid Download Source to Start Me Up !! ⚠️</b>")
                return
            is_file = True
            dl_url = rep_mess
        ...
        await i_m_sefg.edit("<b>⚠️ Opps ⚠️</b>\n\n <b><b>Reply with Direct/Torrent Link or File⁉️</b></b>")
        return
...
async def incoming_youtube_dl_f(client, message):
    ...
    if message.reply_to_message:
        ...
        await i_m_sefg.edit("<b>⚠️ Opps ⚠️</b>\n\n <b><b>Reply To YTDL Supported Link.</b></b>")
        return
    if dl_url is not None:
        await i_m_sefg.edit_text("<code>Extracting Links . . . 🔀</code>")
        # create an unique directory
...
async def rename_tg_file(client, message):
    ...
    if not message.reply_to_message:
        await message.reply("<b>⚠️ Opps ⚠️</b>\n\n <b><b> Reply with Telegram Media (File / Video)⁉️</b>", quote=True)
        return
    if len(message.command) > 1:
        ...
        try:
            ...
            if message_to_send != "":
                mention_req_user = (
                    f"<a href='tg://user?id={usr_id}'><b>📂 Your Uploaded Files !!</b></a>\n\n"
                )
                message_to_send = mention_req_user + message_to_send
                message_to_send = message_to_send + "\n\n" + "#Uploads\n\n<b>💥 <b>Powered By : @whatstarwebshow</b> </b>"
            else:
